...
#include "dummy-vmac-transport.hpp"
...
class VmacLinkServiceFixture : public GlobalIoTimeFixture, public KeyChainFixture
{
protected:
  VmacLinkServiceFixture()
  {
             ...
             ssize_t sendQueueCapacity = QUEUE_UNSUPPORTED)
  {
    face = make_unique<Face>(make_unique<VmacLinkService>(options),
                             make_unique<DummyVmacTransport>("dummy://", "dummy://",
                                                         ndn::nfd::FACE_SCOPE_NON_LOCAL,
                                                         ndn::nfd::FACE_PERSISTENCY_PERSISTENT,
                                                         ndn::nfd::LINK_TYPE_POINT_TO_POINT,
                                                         mtu, sendQueueCapacity));
    service = static_cast<VmacLinkService*>(face->getLinkService());
    transport = static_cast<DummyVmacTransport*>(face->getTransport());
...
protected:
  ...
  DummyVmacTransport* transport = nullptr;
  std::vector<Interest> receivedInterests;
  std::vector<Data> receivedData;
  std::vector<lp::Nack> receivedNacks;
};

BOOST_FIXTURE_TEST_SUITE(TestVmacLinkService, VmacLinkServiceFixture)
...
BOOST_AUTO_TEST_CASE(SendInterest)
{
  ...
  service->sendInterest(*makeInterest("/localhost/test"), 0);
...
BOOST_AUTO_TEST_SUITE_END() // TestVmacLinkService
