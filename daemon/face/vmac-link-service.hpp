...
#include "vmac-reliability.hpp"
#include "vmac-transport.hpp"

namespace nfd {
namespace face {

/** \brief counters provided by VmacLinkService
 *  \note The type name 'VmacLinkServiceCounters' is implementation detail.
 *        Use 'VmacLinkService::Counters' in public API.
 */
...
/** \brief VmacLinkService is a LinkService that implements the NDNLPv2 protocol
...
public:
  ...
  public:
     ...
     */
    VmacReliability::Options reliabilityOptions;
  ...
  /** \brief counters provided by VmacLinkService
   */
  using Counters = VmacLinkServiceCounters;

  explicit
  VmacLinkService(const Options& options = {});

  /** \brief get Options used by VmacLinkService
   */
  const Options&
  getOptions() const;

  /** \brief sets Options used by VmacLinkService
   */
  void
...
PROTECTED_WITH_TESTS_ELSE_PRIVATE: // send path
   ...
   */
  void
  sendLpPacket(lp::Packet&& pkt, const Name name, const TransportFrameType type, const EndpointId& endpointId);
...
private: // send path
   ...
   */
  void
  sendNetPacket(lp::Packet&& pkt, const Name name, const TransportFrameType type, const EndpointId& endpointId, bool isInterest);
...
inline const VmacLinkService::Counters&
...
#endif // NFD_DAEMON_FACE_VMAC_LINK_SERVICE_HPP
