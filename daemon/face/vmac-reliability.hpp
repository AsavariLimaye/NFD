...
#ifndef NFD_DAEMON_FACE_VMAC_RELIABILITY_HPP
#define NFD_DAEMON_FACE_VMAC_RELIABILITY_HPP
...
public:
  ...
  VmacReliability(const Options& options, VmacLinkService* linkService);

  /** \brief signals on Interest dropped by reliability system for exceeding allowed number of retx
   */
  signal::Signal<VmacReliability, Interest> onDroppedInterest;
  ...
  /** \return VmacLinkService that owns this instance
   *
   ...
   */
  void
  handleOutgoing(std::vector<lp::Packet>& frags, lp::Packet&& pkt, bool isInterest, const Name name, const TransportFrameType type);

  /** \brief extract and parse all Acks and add Ack for contained Fragment (if any) to AckQueue
   *  \param pkt incoming LpPacket
   */
  void
  processIncomingPacket(const lp::Packet& pkt, Name name, TransportFrameType type);
...
PUBLIC_WITH_TESTS_ELSE_PRIVATE:
   ...
   */
  std::vector<lp::Sequence>
  onLpPacketLost(lp::Sequence txSeq, const Name name, TransportFrameType type);
...
#endif // NFD_DAEMON_FACE_VMAC_LP_RELIABILITY_HPP
