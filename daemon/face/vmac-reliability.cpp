...
VmacReliability::VmacReliability(const VmacReliability::Options& options, VmacLinkService* linkService)
  : m_options(options)
...
void
VmacReliability::setOptions(const Options& options)
{
...
const VmacLinkService*
VmacReliability::getLinkService() const
{
  return m_linkService;
}

void
VmacReliability::handleOutgoing(std::vector<lp::Packet>& frags, lp::Packet&& pkt, bool isInterest, Name name, TransportFrameType type)
{
  ...
  for (lp::Packet& frag : frags) {
    ...
    unackedFragsIt->second.rtoTimer = getScheduler().schedule(m_rttEst.getEstimatedRto(),
                                                              [=] { onLpPacketLost(txSeq, name, type); });
...
void
VmacReliability::processIncomingPacket(const lp::Packet& pkt, Name name, TransportFrameType type)
{
  BOOST_ASSERT(m_options.isEnabled);
  ...
  for (lp::Sequence ackSeq : pkt.list<lp::AckField>()) {
    ...
    for (lp::Sequence txSeq : lostLpPackets) {
      if (removedLpPackets.find(txSeq) == removedLpPackets.end()) {
        auto removedThisTxSeq = onLpPacketLost(txSeq, name, type);
...
void
VmacReliability::piggyback(lp::Packet& pkt, ssize_t mtu)
{
  BOOST_ASSERT(m_options.isEnabled);
...
lp::Sequence
VmacReliability::assignTxSequence(lp::Packet& frag)
{
...
void
VmacReliability::startIdleAckTimer()
{
  if (m_idleAckTimer) {
...
VmacReliability::findLostLpPackets(VmacReliability::UnackedFrags::iterator ackIt)
{
...
std::vector<lp::Sequence>
VmacReliability::onLpPacketLost(lp::Sequence txSeq, Name name, TransportFrameType type)
{
...
void
VmacReliability::onLpPacketAcknowledged(UnackedFrags::iterator fragIt)
{
...
void
VmacReliability::deleteUnackedFrag(UnackedFrags::iterator fragIt)
{
...
VmacReliability::UnackedFrag::UnackedFrag(lp::Packet pkt)
  : pkt(std::move(pkt))
...
VmacReliability::NetPkt::NetPkt(lp::Packet&& pkt, bool isInterest)
  : pkt(std::move(pkt))
